...
async function run(): Promise<void> {
  ...
  check('fixture decoded', bitmap.width === expected.width && bitmap.height === expected.height,
  ...
  log('');
  log(`session    ${nativeInfo.inW}x${nativeInfo.inH} input (fit ${nativeInfo.fitW}x${nativeInfo.fitH})` +
      `${nativeInfo.fixedInput ? '  [model pinned its own size]' : ''}`);
  log(`runtime    ${nativeInfo.threads} thread(s), simd=${nativeInfo.simd}, isolated=${nativeInfo.isolated}`);
  ...
  check('network input runs at native resolution',
    nativeInfo.inW === 512 && nativeInfo.inH === 288, `${nativeInfo.inW}x${nativeInfo.inH}`);

  const nativeDets = await native.detect(await createImageBitmap(bitmap));
  check('finds every reference face at native resolution',
    nativeDets.length === expected.faces.length, `${nativeDets.length} vs ${expected.faces.length}`);
  ...
  check('boxes reproduce OpenCV through the full pipeline', worstIoU > 0.99,
  ...
  log(`detect     median ${median(timings).toFixed(1)} ms, ` +
      `min ${Math.min(...timings).toFixed(1)}, max ${Math.max(...timings).toFixed(1)} ` +
      `(${RUNS} runs)`);
  log(`           infer ${detector.lastInferMs.toFixed(1)} ms, prep ${detector.lastPrepMs.toFixed(1)} ms`);
  log(`           headroom at 30fps: ${(33.3 - median(timings)).toFixed(1)} ms`);
  log(`           ${dets.length}/${expected.faces.length} faces survive the downscale ` +
  ...
  check('finds the faces that survive the downscale', dets.length >= 3,
  ...
  check('tracker assigns stable ids', t1.length === t2.length &&
    t1.every((t, i) => t.id === t2[i].id), `${t1.map((t) => t.id).join(',')}`);

  const place = placementFor(dets[0]);
  check('roll is level on an upright face', Math.abs(place.roll) < 0.35,
    `${((place.roll * 180) / Math.PI).toFixed(1)} deg`);
  check('eye gap is plausible', place.eyeGap > 5 && place.eyeGap < dets[0].w,
  ...
  check('mirroring negates roll rather than flipping it',
